...
#include "lab_work_project/lab_work_project.hpp"

namespace M3D_ISICG
{
	LabWorkManager::LabWorkManager()
	{
		_current = new LabWorkProject();
		_type	 = TYPE::LAB_WORK_PROJ;
	}
	...
	void LabWorkManager::drawMenu()
	{
		...
		if ( ImGui ::MenuItem( "Lab work Project" ) )
		{
			if ( _type != TYPE ::LAB_WORK_PROJ ) // Change only if needed .
			{
				// Keep window size .
				const int w = _current->getWindowWidth();
				const int h = _current->getWindowHeight();
				delete _current;			  // Delete old lab work .
				_current = new LabWorkProject();	  // Create new lab work .
				_type	 = TYPE ::LAB_WORK_PROJ; // Update type .
				_current->resize( w, h );	  // Update window size .
				_current->init();			  // Don ’t forget to call init ().
			}
		}
