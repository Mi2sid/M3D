...
namespace M3D_ISICG
{
	...
	void Camera::setProjectionP(bool p_projectionP) { _projectionP = p_projectionP; }

	void Camera::setFreeFly( bool p_freeFly ) { _freeFly = p_freeFly; }
	...
	{ 
		if (_freeFly )
			...
			_viewMatrix = glm::lookAt( _position, Vec3f( 0.f ), _up );
	}
	...
	void Camera::_computeProjectionMatrix() { 
		if ( _projectionP )
			...
			_projectionMatrix = glm::ortho( -1.f, 1.f, -1.f, 1.f, _zNear, _zFar );
	}
